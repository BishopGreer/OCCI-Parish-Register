...
class OCCIPR_Parishes {
    ...
    public static function save() {
        if ( ! current_user_can( 'occipr_manage_records' ) || ! check_admin_referer( 'occi_save_parish', 'occi_nonce' ) ) {
        ...
        if ( $id ) {
            $wpdb->update( "{$wpdb->prefix}occipr_parishes", $data, [ 'id' => $id ] );
        } else {
            $wpdb->insert( "{$wpdb->prefix}occipr_parishes", $data );
        }
    ...
    public static function delete() {
        $id = intval( $_GET['id'] ?? 0 );
        if ( ! current_user_can( 'occipr_manage_records' ) || ! check_admin_referer( 'occi_delete_parish_' . $id ) ) {
            wp_die( 'Access denied.' );
        }
        global $wpdb;
