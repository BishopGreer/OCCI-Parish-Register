...
class OCCIPR_Marriage {
    ...
    public static function page() {
        ...
        if ( $action === 'add' || $action === 'edit' ) {
            $r = $id ? $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}occipr_marriages WHERE id=%d", $id ) ) : null;
            self::render_form( $r );
        } elseif ( $action === 'view' && $id ) {
            $r = $wpdb->get_row( $wpdb->prepare( "SELECT m.*, p.name AS parish_name, p.city AS parish_city, p.state AS parish_state FROM {$wpdb->prefix}occipr_marriages m LEFT JOIN {$wpdb->prefix}occipr_parishes p ON m.parish_id=p.id WHERE m.id=%d", $id ) );
            self::render_view( $r );
        } else {
            self::render_list();
        }
    ...
    private static function render_list() {
        ...
        $sql = "SELECT m.*, p.name AS parish_name, p.city AS parish_city FROM {$wpdb->prefix}occipr_marriages m LEFT JOIN {$wpdb->prefix}occipr_parishes p ON m.parish_id=p.id $where ORDER BY m.$orderby $order";
