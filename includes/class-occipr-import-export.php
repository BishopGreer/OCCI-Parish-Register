...
class OCCIPR_ImportExport {
    ...
    public static function handle_export() {
        ...
        if ( $parish_id ) {
            $p = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}occipr_parishes WHERE id = %d", $parish_id ) );
    ...
    private static function export_register( $type, $parish_id ) {
        ...
        switch ( $type ) {
            case 'baptisms':
                $rows = $wpdb->get_results(
                    "SELECT t.*, p.name AS parish_name, p.city AS parish_city, p.state AS parish_state
                     FROM {$wpdb->prefix}occipr_baptisms t
                     LEFT JOIN {$wpdb->prefix}occipr_parishes p ON t.parish_id = p.id
            ...
            case 'confirmations':
                $rows = $wpdb->get_results(
                    "SELECT t.*, p.name AS parish_name, p.city AS parish_city, p.state AS parish_state
                     FROM {$wpdb->prefix}occipr_confirmations t
                     LEFT JOIN {$wpdb->prefix}occipr_parishes p ON t.parish_id = p.id
            ...
            case 'marriages':
                $rows = $wpdb->get_results(
                    "SELECT t.*, p.name AS parish_name, p.city AS parish_city, p.state AS parish_state
                     FROM {$wpdb->prefix}occipr_marriages t
                     LEFT JOIN {$wpdb->prefix}occipr_parishes p ON t.parish_id = p.id
            ...
            case 'deaths':
                $rows = $wpdb->get_results(
                    "SELECT t.*, p.name AS parish_name, p.city AS parish_city, p.state AS parish_state
                     FROM {$wpdb->prefix}occipr_deaths t
                     LEFT JOIN {$wpdb->prefix}occipr_parishes p ON t.parish_id = p.id
            ...
            case 'communions':
                $rows = $wpdb->get_results(
                    "SELECT t.*, p.name AS parish_name, p.city AS parish_city, p.state AS parish_state
                     FROM {$wpdb->prefix}occipr_communions t
                     LEFT JOIN {$wpdb->prefix}occipr_parishes p ON t.parish_id = p.id
            ...
            case 'ordinations':
                $rows = $wpdb->get_results(
                    "SELECT t.*, p.name AS parish_name, p.city AS parish_city, p.state AS parish_state
                     FROM {$wpdb->prefix}occipr_ordinations t
                     LEFT JOIN {$wpdb->prefix}occipr_parishes p ON t.parish_id = p.id
    ...
    private static function import_baptisms( array $records, int $parishes_created ): array {
        ...
        foreach ( $records as $r ) {
            ...
            $ok = $wpdb->insert( "{$wpdb->prefix}occipr_baptisms", $insert );
    ...
    private static function import_confirmations( array $records, int $parishes_created ): array {
        ...
        foreach ( $records as $r ) {
            ...
            $ok = $wpdb->insert( "{$wpdb->prefix}occipr_confirmations", $insert );
    ...
    private static function import_marriages( array $records, int $parishes_created ): array {
        ...
        foreach ( $records as $r ) {
            ...
            $ok = $wpdb->insert( "{$wpdb->prefix}occipr_marriages", $insert );
    ...
    private static function import_deaths( array $records, int $parishes_created ): array {
        ...
        foreach ( $records as $r ) {
            ...
            $ok = $wpdb->insert( "{$wpdb->prefix}occipr_deaths", $insert );
    ...
    private static function import_communions( array $records, int $parishes_created ): array {
        ...
        foreach ( $records as $r ) {
            ...
            $ok = $wpdb->insert( "{$wpdb->prefix}occipr_communions", $insert );
    ...
    private static function import_ordinations( array $records, int $parishes_created ): array {
        ...
        foreach ( $records as $r ) {
            ...
            $ok = $wpdb->insert( "{$wpdb->prefix}occipr_ordinations", $insert );
    ...
    private static function baptism_exists( array $r ): bool {
        ...
        $sql  = "SELECT id FROM {$wpdb->prefix}occipr_baptisms WHERE first_name = %s AND last_name = %s AND baptism_date = %s";
    ...
    private static function confirmation_exists( array $r ): bool {
        ...
        return (bool) $wpdb->get_var( $wpdb->prepare(
            "SELECT id FROM {$wpdb->prefix}occipr_confirmations
    ...
    private static function marriage_exists( array $r ): bool {
        ...
        return (bool) $wpdb->get_var( $wpdb->prepare(
            "SELECT id FROM {$wpdb->prefix}occipr_marriages WHERE marriage_date = %s
    ...
    private static function death_exists( array $r ): bool {
        global $wpdb;
        return (bool) $wpdb->get_var( $wpdb->prepare(
            "SELECT id FROM {$wpdb->prefix}occipr_deaths
    ...
    private static function communion_exists( array $r ): bool {
        global $wpdb;
        return (bool) $wpdb->get_var( $wpdb->prepare(
            "SELECT id FROM {$wpdb->prefix}occipr_communions
    ...
    private static function ordination_exists( array $r ): bool {
        global $wpdb;
        return (bool) $wpdb->get_var( $wpdb->prepare(
            "SELECT id FROM {$wpdb->prefix}occipr_ordinations
    ...
    private static function find_or_create_parish( string $name, string $city, string $state, int &$parishes_created ): ?int {
        ...
        $existing = $wpdb->get_var( $wpdb->prepare(
            "SELECT id FROM {$wpdb->prefix}occipr_parishes WHERE name = %s AND city = %s AND state = %s LIMIT 1",
            $name, $city, $state
        ) );
        if ( $existing ) return intval( $existing );
        // Create it
        $wpdb->insert( "{$wpdb->prefix}occipr_parishes", [
