...
class OCCIPR_Report {
    ...
    private static function render_results( $results, $first, $last ) {
        ...
        foreach ( $sections as $key => [ $label, $page_slug, $cert_type ] ) {
            ...
            foreach ( $results[ $key ] as $r ) {
                ...
                echo ' | ' . OCCIPR_Certificates::certificate_button( $cert_type, $r->id );
                echo '</td>';
                echo '</tr>';
            }
    ...
    public static function print_report() {
        ...
        $issued  = date( 'F j, Y' );
        $font    = OCCIPR_Certificates::get_font();
        $logo    = OCCI_PR_PLUGIN_URL . 'assets/images/certificate-template.png';
