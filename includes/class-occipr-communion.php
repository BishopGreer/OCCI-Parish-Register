...
class OCCIPR_Communion {
    ...
    public static function page() {
        ...
        if ( $action === 'add' || $action === 'edit' ) {
            $r = $id ? $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}occipr_communions WHERE id=%d", $id ) ) : null;
            self::render_form( $r );
        } elseif ( $action === 'view' && $id ) {
            $r = $wpdb->get_row( $wpdb->prepare( "SELECT c.*, p.name AS parish_name, p.city AS parish_city, p.state AS parish_state FROM {$wpdb->prefix}occipr_communions c LEFT JOIN {$wpdb->prefix}occipr_parishes p ON c.parish_id=p.id WHERE c.id=%d", $id ) );
            self::render_view( $r );
        } else {
            self::render_list();
        }
    ...
    private static function render_list() {
        ...
        $sql = "SELECT c.*, p.name AS parish_name FROM {$wpdb->prefix}occipr_communions c LEFT JOIN {$wpdb->prefix}occipr_parishes p ON c.parish_id=p.id $where ORDER BY c.communion_date DESC, c.last_name ASC";
                ...
                <div class="occi-section">
                    ...
                    <table class="form-table">
                        ...
                        <tr><th><label>Parish</label></th>
                            <td><select name="parish_id"><?php echo OCCIPR_Database::parish_dropdown( $r->parish_id ?? 0 ); ?></select></td></tr>
