...
class OCCIPR_Death {
    ...
    public static function page() {
        ...
        if ( $action === 'add' || $action === 'edit' ) {
            $r = $id ? $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}occipr_deaths WHERE id=%d", $id ) ) : null;
            self::render_form( $r );
        } elseif ( $action === 'view' && $id ) {
            $r = $wpdb->get_row( $wpdb->prepare( "SELECT d.*, p.name AS parish_name, p.city AS parish_city, p.state AS parish_state FROM {$wpdb->prefix}occipr_deaths d LEFT JOIN {$wpdb->prefix}occipr_parishes p ON d.parish_id=p.id WHERE d.id=%d", $id ) );
            self::render_view( $r );
        } else {
            self::render_list();
        }
    ...
    private static function render_list() {
        ...
        $sql = "SELECT d.*, p.name AS parish_name FROM {$wpdb->prefix}occipr_deaths d LEFT JOIN {$wpdb->prefix}occipr_parishes p ON d.parish_id=p.id $where ORDER BY d.death_date DESC";
